...
public class VideoCalls extends AppCompatActivity {
    ...
    //participant value
    private String remoteParticipantUid;
    private User currentUser;
    private String currentUid;

    //activity state
    public static boolean isMakingVideoCall = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //remote participant
        remoteParticipantUid = getIntent().getStringExtra(Common.CALL_USER_INTENT);
    ...
    private void intializeUI() {
        ...
        //participant name
        DocumentReference userRef = FirebaseFirestore.getInstance().collection(Common.USER_NODE)
                .document(remoteParticipantUid);
        userRef.get()
                .addOnCompleteListener(task -> {

                    if (task.isSuccessful()){

                        participantName.setText(task.getResult().get("first_name").toString() + " " + task.getResult().get("last_name").toString());
    ...
    private void callParticipant(String remoteParticipantUid) {

        //get the user fcm token
        DocumentReference userRef = FirebaseFirestore.getInstance().collection(Common.USER_NODE).document(remoteParticipantUid);
        userRef.get()
                .addOnCompleteListener(task2 -> {

                    if (task2.isSuccessful()){

                        User theParticipant = task2.getResult().toObject(User.class);
                        String participantFcmTokenId = theParticipant.getFcm_token();

                        //check if token is empty
                        if (!TextUtils.isEmpty(participantFcmTokenId)){


                            //create call document id
                            DocumentReference pushRef = FirebaseFirestore.getInstance().collection(Common.CALL_SESSION_NODE).document();
                            String pushId = pushRef.getId();

                            //create session
                            CallSession newSession = new CallSession(pushId, currentUid, remoteParticipantUid, Common.CALL_CONNECTING_STATUS);

                            //push
                            DocumentReference sessionRef = FirebaseFirestore.getInstance().collection(Common.CALL_SESSION_NODE).document(pushId);
                            sessionRef.set(newSession)
                                    .addOnCompleteListener(task -> {
                                        ...
                                        if (task.isSuccessful()){
                                            ...
                                            String roomName = currentUid;
                                            String theTitle = "Video Call";
                                            String theMessage = name +" is inviting you to a video session.";
                                            String serverKey = "key=" + Paper.book().read(Common.SERVER_KEY);

                                            //set header map
                                            Map<String, String> headerMap = new HashMap<>();
                                            headerMap.put("Content-Type", "application/json");
                                            headerMap.put("Authorization", serverKey);

                                            //set notification map
                                            Map<String, String> notificationMap = new HashMap<>();
                                            notificationMap.put("title", theTitle);
                                            notificationMap.put("message", theMessage);
                                            notificationMap.put("data_type", "Video Call");
                                            notificationMap.put("user_id", userId);
                                            notificationMap.put("user_name", name);
                                            notificationMap.put("room_name", roomName);
                                            notificationMap.put("call_session_id", pushId);

                                            //set firebase messaging data
                                            FirebaseCloudMessage cloudMessage = new FirebaseCloudMessage(participantFcmTokenId, notificationMap);

                                            //send notification
                                            Call<ResponseBody> call =Common.getFCMService().send(headerMap, cloudMessage);
                                            call.enqueue(new retrofit2.Callback<ResponseBody>() {
                                                ...
                                                public void onFailure(Call<ResponseBody> call, Throwable t) {
                                                    ...
                                                    Log.d("Send Message Notify", "error: " + t.getMessage());
                                                    Toast.makeText(VideoCalls.this, "Connection Failed", Toast.LENGTH_SHORT).show();
                                            ...
                                            DocumentReference theSessionRef = FirebaseFirestore.getInstance().collection(Common.CALL_SESSION_NODE)
                                                    .document(pushId);
                                            theSessionRef.addSnapshotListener(VideoCalls.this, (documentSnapshot, e) -> {

                                                if (e == null){

                                                    CallSession theSession = documentSnapshot.toObject(CallSession.class);

                                                    //check

                                                    if (!documentSnapshot.exists()){

                                                        endCallSession(pushId, "");

                                                    } else {

                                                        if (theSession.getSession_status().equals(Common.CALL_BUSY_STATUS)) {

                                                            endCallSession(pushId, Common.CALL_BUSY_STATUS);

                                                        }

                                                        if (theSession.getSession_status().equals(Common.CALL_DECLINED_STATUS)) {

                                                            endCallSession(pushId, Common.CALL_DECLINED_STATUS);

                                                        }

                                                        if (theSession.getSession_status().equals(Common.CALL_CONNECTED_STATUS)) {

                                                            //connect
                                                            connectToRoom(Paper.book().read(Common.USER_ID));
                                                    ...
                                                    new Handler().postDelayed(() -> {
                                                        ...
                                                        DocumentReference checkSessRef = FirebaseFirestore.getInstance().collection(Common.CALL_SESSION_NODE)
                                                                .document(pushId);
                                                        checkSessRef.get()
                                                                .addOnCompleteListener(task3 -> {
                                                                    ...
                                                                    if (task3.isSuccessful()){
                                                                        ...
                                                                        if (task3.getResult().exists()) {
                                                                            ...
                                                                            CallSession checkSess = task3.getResult().toObject(CallSession.class);
                                                                            if (!checkSess.getSession_status().equals(Common.CALL_CONNECTED_STATUS)) {

                                                                                endCallSession(pushId, Common.CALL_TIMEOUT_STATUS);
    ...
    private void endCallSession (String pushId, String status) {

        //show status
        videoStatusTextView.setText(status);
    ...
    private Task<String> getAuthToken() {
        ...
        data.put("room", Paper.book().read(Common.USER_ID));
        ...
        return mFunctions
                ...
                .continueWith(task -> {
                    ...
                    //call user
                    callParticipant(remoteParticipantUid);
        ...
        return new RemoteParticipant.Listener() {
            ...
            public void onVideoTrackSubscriptionFailed(RemoteParticipant remoteParticipant,
                ...
                Log.i(TAG, String.format("onVideoTrackSubscriptionFailed: " +
                        ...
                        twilioException.getCode(),
                        twilioException.getMessage()));
                Toast.makeText(VideoCalls.this, "Failed to subscribe to %s video track", Toast.LENGTH_SHORT).show();
            }
